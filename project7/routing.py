...
def find_paths(grid, points):
    ...
    for(source, target) in points:
        path = bfs(grid, source, target, count)
        paths.append(path[::-1])
        for point in path:
            grid.d[point] = count
        count += 1
...
def bfs(grid,source,target,pathnum):
    g = Graph(copy.deepcopy(grid))
    ends = deque()
    ends.append(source)
    ...
    while len(ends) > 0:
        ...
        if coord == target:
            ...
            for dot in g.adj(coord):
                if g.getVal(dot) == 0 or dot == target:
                    g.putVal(dot, pathnum)
                    ends.append(dot)
                    parent[dot] = coord
    res = []
    c = target
